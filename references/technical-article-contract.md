...
- Each framework/runtime third-level directory has one reserved non-API exception: `三方库/index.md`. Its frontmatter sets `kind: "ecosystem-libraries"` and `category` to the third-level framework/runtime path. Place it last in that framework's sidebar menu.
- "三方库" means an external package associated with or used alongside the framework; never list the framework/runtime package itself. Build this page from the latest `resources/sessions/` cache. Its `三方库列表` table must have `库名｜为何使用｜如何使用` columns. State `暂未识别关联三方库` when the cache contains no external-package evidence.
