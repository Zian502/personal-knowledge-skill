...
- Put a third-party library's package-level or sole entry API directly at
  `技术/<领域>/<框架>/三方库/<库名>/<api-slug>/index.md`, with `category`
  ending at `<库名>`. Do not create a generic intermediate module solely to
  hold one API. Add `<模块>` only when the library genuinely exposes multiple
  reusable modules, using `.../<库名>/<模块>/<api-slug>/index.md`. For example,
  `electron-window-state/windowstatekeeper/` is direct, while Effect retains
  its `Deferred` and `Fiber` module directories.
